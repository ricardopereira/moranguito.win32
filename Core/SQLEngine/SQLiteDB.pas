...
type
  ...
  TSQLiteDatabase = class(TPersistent)
  private
    fDB: TSQLiteDB;
    fInTrans: boolean;
    fSync: boolean;
  ...
  protected
    procedure LoadFromSql(DB: TSQLiteDatabase; const SQL: string; const Bindings: array of const); virtual;
  public
    ...
    function ColumnExist(ColumnName: string): boolean;
    ...
    case Bindings[I].VType of
      ...
      begin
        case Bindings[I].VType of
          ...
          vtChar: begin
            DataPtr := PAnsiChar(Bindings[I].VChar);
            DataSize := 2;
          end;
          vtWideChar: begin
...
begin
  try
    if Sqlite3_Prepare_v2(self.fDB, PAnsiChar(AnsiString(SQL)), -1, Stmt, NextSQLStatement) <>
      SQLITE_OK then
...
function TSQLiteTable.ColumnExist(ColumnName: string): boolean;
begin
  result := true;
  if (fCols = nil) then result := false;
  if (fCols.count = 0) then result := false;
  if (fCols.IndexOf(AnsiUpperCase(ColumnName))<0) then result := false;
end;
...
procedure TSQLiteTable.LoadFromSql(DB: TSQLiteDatabase; const SQL: string; const Bindings: array of const);
var
  Stmt: TSQLiteStmt;
  NextSQLStatement: PAnsiChar;
  iStepResult: integer;
